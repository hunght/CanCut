...
import { getAppDefaultDownloadPath, ensureDownloadPathAccessible } from "@/helpers/downloadPath";
...
const getDefaultDownloadPath = (): string => {
  return getAppDefaultDownloadPath();
};
...
export const preferencesRouter = t.router({
  ...
  getDownloadPath: publicProcedure.query(async ({ ctx }): Promise<GetDownloadPathResult> => {
    ...
    try {
      ...
      let downloadPath = customPath || defaultPath;
      let isDefault = !customPath;

      try {
        await ensureDownloadPathAccessible(downloadPath);
      } catch (error) {
        if (customPath) {
          logger.warn("[preferences] Custom download path inaccessible, falling back to default", {
            customPath,
    ...
    .mutation(async ({ input, ctx }): Promise<UpdateDownloadPathResult> => {
      ...
      try {
        if (input.downloadPath) {
          await ensureDownloadPathAccessible(input.downloadPath);
        }
