...
app.whenReady().then(async () => {
  ...
  protocol.registerFileProtocol("local-file", (request, callback) => {
    ...
    try {
      ...
      const decodedPath = decodeURIComponent(originalUrl.replace("local-file://", ""));
      const normalizedPath = path.normalize(decodedPath);
      const exists = fs.existsSync(normalizedPath);

      logger.info("[protocol][local-file] Resolving request", {
        url: originalUrl,
      ...
      logger.error("[protocol] Failed to resolve local-file URL", {
        url: originalUrl,
        error: e,
      });
