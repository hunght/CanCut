...
import { ensureDownloadPathAccessible, getAppDefaultDownloadPath } from "@/helpers/downloadPath";
...
const getDownloadPath = async (db: Database): Promise<string> => {
  const defaultPath = getAppDefaultDownloadPath();
  ...
  try {
    ...
    if (prefs.length > 0 && prefs[0].downloadPath) {
      customPath = prefs[0].downloadPath;
    }
  } catch (error) {
    logger.warn("[queue-manager] Failed to get download path from preferences", { error });
  }

  const downloadPath = customPath || defaultPath;

  try {
    await ensureDownloadPathAccessible(downloadPath);
    return downloadPath;
  } catch (error) {
    logger.error("[queue-manager] Download path inaccessible", {
      downloadPath,
      error: error instanceof Error ? error.message : error,
    });

    if (customPath) {
      logger.warn("[queue-manager] Falling back to default download path", { defaultPath });
      await ensureDownloadPathAccessible(defaultPath);
      return defaultPath;
    }
...
const createQueueManager = (
  ...
  const markCompleted = async (
    ...
    if (item.videoId) {
      logger.info("[queue-manager] Persisting completed download", {
        downloadId,
        videoId: item.videoId,
        filePath,
      });
      ...
      logger.warn("[queue-manager] markCompleted called without videoId", { downloadId, filePath });
    }
