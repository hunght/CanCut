      ...
      if (code === 0) {
        ...
        if (!finalPath && w.videoId && w.outputDir && fs.existsSync(w.outputDir)) {
          try {
            ...
            if (match) {
              finalPath = path.join(w.outputDir, match);
              logger.info("[download-worker] Resolved file via directory scan", {
                downloadId,
                videoId: w.videoId,
                finalPath,
              });
            ...
            logger.warn("[download-worker] Failed to scan output directory for file", {
              downloadId,
              videoId: w.videoId,
              outputDir: w.outputDir,
              error: scanError instanceof Error ? scanError.message : scanError,
            });
          }
        }
        if (!finalPath) {
          logger.warn("[download-worker] Unable to determine final path, falling back to template", {
            downloadId,
            videoId: w.videoId,
            outputPath,
            lastKnownFilePath: w.lastKnownFilePath,
          });
        }
        logger.info("[download-worker] Notifying queue manager download completed", {
          downloadId,
          videoId: w.videoId,
...
const parseProgressAndMetadata = (db: Database, downloadId: string, output: string): void => {
  ...
  if (foundPath) {
    ...
    if (worker) {
      ...
      logger.debug("[download-worker] Captured destination from yt-dlp output", {
        downloadId,
        videoId: worker.videoId,
        path: cleaned,
        raw: foundPath,
      });
    }
